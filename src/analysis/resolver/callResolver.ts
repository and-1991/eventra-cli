...
function resolveDeclaration(declaration: ts.Declaration): ts.FunctionLikeDeclaration | null {
  // function foo() {}
...
function getExpressionSymbol(expression: ts.Expression, checker: ts.TypeChecker): ts.Symbol | undefined {
  // analytics.track()
...
export function resolveFunctionFromCall(expression: ts.Expression, checker: ts.TypeChecker, callCache: ResolvedCallCache, exportCache: ResolvedExportCache): ts.FunctionLikeDeclaration | null {
  let symbol = getExpressionSymbol(expression, checker);
  if (!symbol) {
    return null;
  }
  // export unwrap
  ...
  // cache
  const cached = callCache.get(symbol);
  if (cached !== undefined) {
    return cached;
  }
  ...
  for (const declaration of declarations) {
    const resolved = resolveDeclaration(declaration);
    if (!resolved) {
      continue;
    }
    callCache.set(symbol, resolved);
    return resolved;
  }
  // fallback:
  // type signatures

  const type = checker.getTypeOfSymbolAtLocation(symbol, declarations[0] ?? expression);
  const signatures = checker.getSignaturesOfType(type, ts.SignatureKind.Call);
  for (const signature of signatures) {
    const declaration = signature.declaration;
    if (declaration && isFunctionLike(declaration)) {
      callCache.set(symbol, declaration);
      return declaration;
    }
  }
  // unresolved
  callCache.set(symbol, null);
  return null;
}
